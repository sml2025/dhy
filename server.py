...
# 数据库配置
# 数据库配置（使用绝对路径，避免因工作目录变化导致无法打开数据库文件）
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATABASE = os.environ.get('DATABASE_PATH', os.path.join(BASE_DIR, 'consultations.db'))
...
def init_db():
    ...
    if not table_exists:
        ...
        # 兼容早期字段命名：text -> contact，description -> message
        if 'text' in columns and 'contact' not in columns:
            cursor.execute('ALTER TABLE consultations RENAME COLUMN text TO contact')
            columns.remove('text')
            columns.append('contact')
        if 'description' in columns and 'message' not in columns:
            cursor.execute('ALTER TABLE consultations RENAME COLUMN description TO message')
...
def send_email(consultation_data):
    """发送邮件到指定邮箱"""
    try:
        # 邮件配置（使用环境变量，避免将凭证硬编码到仓库）
        sender_email = os.environ.get("SENDER_EMAIL")
        sender_password = os.environ.get("SENDER_PASSWORD")
        receiver_email = os.environ.get("RECIPIENT_EMAIL", sender_email)

        if not sender_email or not sender_password:
            print("发送邮件跳过: 未配置 SENDER_EMAIL 或 SENDER_PASSWORD 环境变量")
            # 不阻塞表单提交流程
            return True
...
# 健康检查（用于 Render 健康探测与公网自检）
@app.route('/health')
def health():
    return jsonify({"status": "healthy", "message": "SML25 Backend is running"})
